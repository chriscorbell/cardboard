...
date: 2026-09-14
---
# Two GitHub Apps separate what a Session can push from what Cardboard can merge
...
request after Cardboard says so", and a prompt or an MCP permission check cannot govern a direct
API call. We therefore use two GitHub Apps with the same repository permissions: "Cardboard
Sessions" mints one-hour installation tokens for Sessions, and "Cardboard Merge" is used only by
...
attempt is refused by GitHub, not by Cardboard.

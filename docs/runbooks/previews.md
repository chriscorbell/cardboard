...
Before the rebrand, the Cardboard Board was switched to runner mode. Its runner cloned the branch for [PR 11](https://github.com/chriscorbell/kardboard/pull/11), built the root Dockerfile, and started a healthy preview on port 3000 with no host mounts or credential variables. Chris's existing Admin sign-in opened [the preview](https://mq729nev.kardboard.cc/) in the browser and showed its separate demo database. [PR 12](https://github.com/chriscorbell/kardboard/pull/12) includes those build fixes and the rebrand. Other branches inherit the root Dockerfile after that change merges. The live preview hostname moved to `mq729nev.kardboard.cc`.

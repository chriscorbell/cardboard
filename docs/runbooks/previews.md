...
Before the rebrand, the Cardboard Board was switched to runner mode. Its runner cloned the branch for [PR 11](https://github.com/chriscorbell/cardboard/pull/11), built the root Dockerfile, and started a healthy preview on port 3000 with no host mounts or credential variables. Chris's existing Admin sign-in opened [the preview](https://mq729nev-preview.xode.cc/) in the browser and showed its separate demo database. Other branches inherit the root Dockerfile after PR 11 merges.

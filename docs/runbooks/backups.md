...
| --- | --- |
| `cardboard.db`, `cardboard.db-wal`, `cardboard.db-shm` | The live database. Never copy these while the app runs |
| `backups/cardboard-<UTC stamp>.db` | Snapshots. Each one is a complete, self-contained database |
| `uploads/<xx>/<sha256>` | Comment attachments, content-addressed and immutable |

Copying `cardboard.db` alone is not a backup. Recent commits live in the write-ahead log until a checkpoint, so a bare copy is typically a nearly empty database: on a freshly seeded instance the main file was 4 KB against a 313 KB WAL, and the copy could not read a single table.
...
One snapshot is taken per day, at `CARDBOARD_BACKUP_HOUR` in the server's timezone. The schedule holds no state in memory: on every tick the app compares the newest snapshot on disk with the last time the scheduled hour passed, so a restart, a deploy, or hours of downtime still produce the missed snapshot as soon as the app is back. After a successful snapshot the oldest files beyond `CARDBOARD_BACKUP_KEEP` are removed, along with any `.partial` file left behind by an interrupted run.

| Variable | Default | Meaning |
| --- | --- | --- |
| `CARDBOARD_BACKUP_HOUR` | `4` | Local hour of the daily snapshot. Negative turns scheduled snapshots off |
| `CARDBOARD_BACKUP_KEEP` | `14` | How many snapshots to keep |
| `CARDBOARD_BACKUP_DIR` | `<data dir>/backups` | Where snapshots are written |
...
```bash
cd ~/docker/stacks/cardboard
docker compose exec app node --input-type=module -e "import {createClient} from '@libsql/client'; const c = createClient({ url: 'file:/data/backups/cardboard-20260914T040000Z.db' }); console.log((await c.execute('PRAGMA integrity_check')).rows[0], (await c.execute('SELECT count(*) AS cards FROM cards')).rows[0]); c.close();"
```
...
cd ~/docker/data/cardboard/app
mv cardboard.db cardboard.db.broken          # keep the damaged files until the restore is confirmed
mv cardboard.db-wal cardboard.db-wal.broken 2>/dev/null
mv cardboard.db-shm cardboard.db-shm.broken 2>/dev/null

cp backups/cardboard-20260914T040000Z.db cardboard.db
chown --reference=cardboard.db.broken cardboard.db   # the container runs as uid 1000; prefix sudo if you are not the owner
...
- **Snapshots share the disk with the database.** They survive a corrupted database or a bad deploy, not a lost disk. Copying them off the host is a manual step and remains out of scope for v1, see [the design](../design.md#out-of-scope-for-v1). An `scp` or `rsync` of `~/docker/data/cardboard/app/backups` and `uploads` from another machine is enough:

  ```bash
  rsync -a minicore:docker/data/cardboard/app/{backups,uploads} ~/backups/cardboard/
  ```

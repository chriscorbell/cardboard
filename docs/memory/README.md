...
2026-09-14: ordinary review after implementing verified SQLite snapshots (card "Backups"). `context/minicore-deployment-constraints.md` deferred again: its live fleet survey exceeds the incidental review bound and this task changed no infrastructure, so its verification date is unchanged. `lessons/pnpm-11-build-approvals.md` checked against `pnpm-workspace.yaml`, `.npmrc`, and a passing `pnpm install --frozen-lockfile` in this Session container; retained, date unchanged because it already reads 2026-09-14. `work/2026-09-14-first-implementation.md` checked and retained: its next action (design-review findings 3, 5, and 6) is still open, and its note that this repository has no per-pull-request Preview still matches `.github/workflows/ci.yml`. Added `work/agents-md-proposal-acceptance-command-tests.md`; the backup mechanism itself is documented in [the backups runbook](../runbooks/backups.md) rather than memory. Next cursor: `context/` entry 1.

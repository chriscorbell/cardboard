...
Next action: raised with @chris on card `v5u4wwzpucs73k` (Backups), together with the CI step, which needs a hand with `workflows` permission. On a yes, edit the block in `AGENTS.md` and delete this note and its index entry.

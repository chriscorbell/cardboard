...
PR 12 combines the rebrand and PR 11 preview setup fixes. Its remaining gate is GitHub's required approving review; the interactive owner has an Admin bypass, but no merge has been performed. Rebuild Card mq729n's Preview from the combined branch and verify it, then obtain the final release authorization. After merge, wait for all five CI image publications and Watchtower deployment; verify the live lowercase wordmark, redirects, and authorized preview access. Stop the local QA server. Record the release revision and close this note when the live code matches the new configuration.

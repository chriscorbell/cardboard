...
- The proxy's memory of a refusal is a `Map` in the process, not a row. Restarting the egress container forgets every window, and the app's response to that is to behave as though no Provider is limited. That is the intended failure direction: `readProviderLimits` never throws and returns "nothing seen" for a timeout, a bad body, or an unset `CARDBOARD_EGRESS_URL`.
- `/limits` is guarded by `EGRESS_CONTROL_TOKEN`, which `deploy/compose.yaml` sets to `CARDBOARD_RUNNER_TOKEN`. That reuse is deliberate and load-bearing: Session containers are on the `workload` network and can reach the proxy, and they are the one thing that never holds the runner token.
...
- A Codex Session started with the sign-in file mounted (no `CARDBOARD_CODEX_VIA_EGRESS`) talks to the provider directly. Its refusals never reach the proxy, so fallback out of Codex cannot work on such a host, and fallback *into* it would put a credential in a container. Turn the Admin setting off there.

...
Unverified caveat: a pull request that touches a workflow file may also be unmergeable by the merge App, which holds the same three permissions and merges through the API. If a workflow change ever does reach a pull request, expect the Approval merge to fail the same way and check before promising otherwise.

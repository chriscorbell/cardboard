...
| `preview-router` | Routes `*.preview.xode.cc` by hostname and enforces the signed cookie | Host port behind the tunnel; `control`, `preview` |
...
Public exposure uses the existing Cloudflare Tunnel with two hostnames added in the Cloudflare dashboard: `cardboard.xode.cc` and the Preview wildcard, `*.preview.xode.cc` or `*-preview.xode.cc` depending on the certificate the zone has. Clerk is the only gate; there is no Cloudflare Access. TLS terminates at Cloudflare's edge.

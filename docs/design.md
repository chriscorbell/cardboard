...
Repositories live on GitHub. One GitHub App is installed per repository, and each Session receives a one-hour installation token scoped to that repository. Commits and merges are attributed to the App's bot identity. See [ADR 0007](adr/0007-github-app-installation-tokens-per-session.md).

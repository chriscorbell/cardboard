...
The hostname shape is configurable with `CARDBOARD_PREVIEW_HOST_PATTERN` and defaults to `{card}.preview.{domain}`, giving `<card>.preview.xode.cc`. That is a second-level subdomain, which Cloudflare Universal SSL does not cover; a zone without an advanced certificate should use `{card}-preview.{domain}` instead, which it does cover.

Preview access never relies on a domain-wide cookie. The app has no session cookie at all: the front end holds a Clerk token and sends it as a bearer header, so there is no ambient credential for a Preview host to receive. Opening a Preview host without a Preview cookie redirects to the app, which checks current Board membership and redirects back with a short-lived, single-use authorization code bound to that Board and that Preview. The preview router exchanges the code for a host-only Preview cookie scoped to that one Preview host, valid for a few hours, and strips every cookie and authorization header before proxying to branch-controlled code. Revoking a Member's access to a Board, or revoking the user outright, raises a Preview epoch that invalidates outstanding Preview cookies for that Board.
...
Public exposure uses the existing Cloudflare Tunnel. The exact `cardboard.xode.cc` rule points to port 3070; a later `*.xode.cc` rule points to the preview router on port 3073. Preview URLs use `{card}-preview.xode.cc`, covered by the existing Universal SSL certificate. A proxied wildcard DNS record points to that tunnel. `*-preview.xode.cc` is not a working wildcard. Clerk is the only gate; there is no Cloudflare Access. TLS terminates at Cloudflare's edge.
...
These cannot be automated from inside Cardboard: adding the two tunnel hostnames in the Cloudflare dashboard and choosing the Preview hostname shape the zone's certificate covers (an advanced certificate for `*.preview.xode.cc`, or `CARDBOARD_PREVIEW_HOST_PATTERN={card}-preview.{domain}` with Universal SSL), setting `CARDBOARD_PREVIEW_SECRET` in `deploy/.env`, verifying `cardboard.xode.cc` in Resend, creating the two GitHub Apps and installing both on each repository ([deploy/github-apps.md](../deploy/github-apps.md)), creating the Clerk application (a secondary application on `cardboard.xode.cc` with Google sign-in), and generating the Claude Code long-lived token with `claude setup-token`. Preparing a repository, including its branch ruleset, is scripted for an agent in the `cardboard-onboard` skill in [chriscorbell/skills](https://github.com/chriscorbell/skills/tree/main/cardboard-onboard).

## Status

As of 2026-09-14 the stack runs on minicore at `https://cardboard.xode.cc` and the loop has completed on two repositories, including this one: card, Session, pull request, Approval, merge by Cardboard, deploy. Runner-hosted Previews were configured on 2026-09-15 UTC for Card mq729n, and the Cardboard Board uses runner mode. Public HTTPS checks with a temporary Member verified code exchange, host-only cookies, credential stripping, and membership revocation. A real Member's Clerk sign-in remains unverified. [The preview runbook](runbooks/previews.md) records configuration and verification limits. Not yet built: child-Card dispatch. Both Providers run with their credential in the egress proxy, and Provider fallback on usage limits was built on 2026-09-15; no real 429 has been seen through the deployed proxy yet. [docs/design-review.md](design-review.md) records which review findings are resolved.

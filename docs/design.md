...
The hostname shape is configurable with `CARDBOARD_PREVIEW_HOST_PATTERN` and defaults to `{card}.{domain}`, giving `<card>.kardboard.cc`. These first-level hosts fit Cloudflare Universal SSL.
...
kardboard runs on minicore as one compose stack named `cardboard` in `chriscorbell/stacks`, with data under `/home/chris/docker/data/cardboard`:

...
The full loop runs in production: sign-in, card to Session, pull request, Approval, merge, deploy. Still to come: runner-hosted previews, provider fallback on usage limits, and invitation emails. Codex through the credential proxy is built but not yet proven against the real backend. See [docs/design.md](docs/design.md) for the current status and [docs/runbooks](docs/runbooks/) for operations.

...
const ADMIN_EMAIL = process.env.CARDBOARD_ADMIN_EMAIL ?? "hi@chriscorbell.com";
const ADMIN_NAME = process.env.CARDBOARD_ADMIN_NAME ?? "Chris Corbell";
...
async function seedDemo(): Promise<void> {
  ...
  const cards: C[] = [
    ...
    { id: newId(), boardId: lumen.id, title: "Case study page for the Meridian rebrand", description: "Long-form page with before/after, three pull quotes, and the process timeline. Copy is final in the doc linked below.", priority: "medium", column: "in_progress", position: 1000, creatorKind: "user", creatorId: tomasz!.id, branch: "cardboard/meridian-case-study", createdAt: minutesAgo(90), updatedAt: minutesAgo(12) },
    { id: newId(), boardId: lumen.id, title: "Footer newsletter signup", description: "Small email field in the footer that posts to our Buttondown list.", priority: "low", column: "review", position: 1000, creatorKind: "user", creatorId: priya!.id, branch: "cardboard/footer-newsletter", prUrl: "https://github.com/chriscorbell/lumen-site/pull/41", prNumber: 41, previewUrl: "https://footer-newsletter.lumen-site.pages.dev", createdAt: minutesAgo(1500), updatedAt: minutesAgo(45) },
    { id: newId(), boardId: lumen.id, title: "Fix the 404 page layout on mobile", description: "The illustration overflows the viewport on phones.", priority: "none", column: "done", position: 1000, creatorKind: "user", creatorId: tomasz!.id, branch: "cardboard/404-mobile", prUrl: "https://github.com/chriscorbell/lumen-site/pull/38", prNumber: 38, createdAt: minutesAgo(4000), updatedAt: minutesAgo(2000) },
  ...
  await db.insert(schema.sessions).values([
    { id: newId(), boardId: lumen.id, cardId: meridian!.id, kind: "card", provider: "claude", status: "running", intent: "Build the Meridian case study page from the final copy doc; touching src/pages/work and the shared timeline component.", branch: "cardboard/meridian-case-study", startedAt: minutesAgo(12), createdAt: minutesAgo(12) },
    { id: newId(), boardId: lumen.id, cardId: footer!.id, kind: "card", provider: "claude", status: "succeeded", intent: "Add footer newsletter signup posting to Buttondown.", branch: "cardboard/footer-newsletter", startedAt: minutesAgo(70), endedAt: minutesAgo(45), outcomeSummary: "Opened PR #41 with a preview and moved the card to Review.", createdAt: minutesAgo(70) },

...
async function startSweep(board: typeof schema.boards.$inferSelect, wallClockMinutes: number): Promise<void> {
  ...
  try {
    const { containerId } = await runner.start({ sessionId, boardSlug: board.slug, provider: board.provider, image: board.agentImage, repoUrl: null, branch: null, token, wallClockMinutes, prompt, githubToken: null, gitName: "cardboard", gitEmail: "cardboard@users.noreply.github.com" });

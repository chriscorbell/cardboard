...
async function startSweep(board: typeof schema.boards.$inferSelect, wallClockMinutes: number): Promise<void> {
  ...
  try {
    const { containerId } = await runner.start({ sessionId, boardSlug: board.slug, provider: board.provider, image: board.agentImage, repoUrl: board.repoUrl, branch: null, token, wallClockMinutes, prompt });

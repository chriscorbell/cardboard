...
async function tick(): Promise<void> {
  ...
  for (const board of boards) {
    ...
    await startSweep(board, settings.sessionWallClockMinutes);
  }
}

async function startSweep(board: typeof schema.boards.$inferSelect, wallClockMinutes: number): Promise<void> {
  ...
  const prompt = `You are running the nightly hygiene sweep for the "${board.name}" board in Cardboard. Read the ledger and the board. For every card, check that its column matches its state: Blocked cards wait on a human, Ready cards are triaged and unclaimed, In Progress cards have an active session or an open branch, Review cards have a pull request and preview, Done cards are merged or closed. Move cards that drifted, explaining each move in a one-line comment. For cards that have sat in Blocked for 14 days with no human reply, post one reminder mentioning the card's author. Do not open pull requests or change code. Finish with a one-sentence summary.${board.promptAppend ? `\n\nBoard-specific instructions from the Admin:\n${board.promptAppend}` : ""}`;
  try {
    const { containerId } = await runner.start({ sessionId, boardSlug: board.slug, provider: board.provider, model: board.model, reasoning: board.reasoning, image: board.agentImage, repoUrl: null, branch: null, token, wallClockMinutes, prompt, githubToken: null, gitName: "cardboard", gitEmail: "cardboard@users.noreply.github.com" });
    ...
    publish(board.id, { type: "session.updated", session: { id: row.id, kind: row.kind, status: row.status, provider: row.provider, intent: row.intent, branch: row.branch, cardId: row.cardId, startedAt: row.startedAt, endedAt: row.endedAt, outcomeSummary: row.outcomeSummary, createdAt: row.createdAt } });
